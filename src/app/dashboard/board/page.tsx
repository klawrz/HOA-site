...
import { getBoardIssueAlerts } from "@/lib/board-issues"
import { BoardIssuesAlerts } from "@/components/board/board-issues-alerts"
...
export default async function BoardDashboard() {
  ...
  const [agm, minutesPending, attentionItems, financialAlerts, issueAlerts, recentMeetings, recentDocs, ownBoardPosition] = await Promise.all([
    session.user.orgId
    ...
    session.user.orgId ? getBoardIssueAlerts(session.user.orgId, "/dashboard/board") : Promise.resolve([]),
  ...
  return (
    <div className="space-y-6">
      ...
      <BoardIssuesAlerts alerts={issueAlerts} />
